...
// Return a base64 encoded placeholder image with a 4px by 4px size
export const blurhashDecode = (blurhash: string) => {
  return blurHashToDataURL(blurhash, 4, 4)
}
...
const createCRCTable = (CRC_TABLE: number[]) => {
  let n, c, k
  for (n = 0; n < 256; n++) {
    c = n
    for (k = 0; k < 8; k++) {
...
function createChunk(length: number, type: string, data: string, CRC_TABLE: number[]) {
  const CRC = crc(type + data, CRC_TABLE)
  return dwordAsString(length) + type + data + dwordAsString(CRC)
}

const updateCRC = (crc: number, buf: string, CRC_TABLE: number[]) => {
  let c = crc
  let b: number
...
const createIHDR = (width: number, height: number, CRC_TABLE: number[]) => {
  const IHDRdata =
    dwordAsString(width) +
...
function generatePng(width: number, height: number, rgbaString: string) {
  ...
  const IDAT = createChunk(compressedScanlines.length, 'IDAT', compressedScanlines, CRC_TABLE)
